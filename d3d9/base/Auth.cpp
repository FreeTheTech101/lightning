...
bool Auth_DisplayLoginDialogDownLevel(const wchar_t** usernameP, const wchar_t** passwordP, PCREDUI_INFOW info)
{
	...
	HRESULT result = CredUIPromptForCredentialsW(info, L"triobit.net", NULL, NULL, username, CREDUI_MAX_USERNAME_LENGTH, password, CREDUI_MAX_PASSWORD_LENGTH, &save, CREDUI_FLAGS_GENERIC_CREDENTIALS | CREDUI_FLAGS_ALWAYS_SHOW_UI | CREDUI_FLAGS_DO_NOT_PERSIST | CREDUI_FLAGS_EXCLUDE_CERTIFICATES);
...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	if (!doneFile && (err = fopen_s(&iw4mini, "lightning.ini","r")) == 0){
	...
	info.pszCaptionText = L"Authentication for Lightning";
	info.pszMessageText = L"Please log in to use Lightning. Use your triobit.net forum credentials.";
	...
	if (result == ERROR_SUCCESS)
	{
		...
		*passwordP = password;
		...
		if(save && (err = fopen_s(&iw4mini, "lightning.ini","w")) == 0){
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://%s/remauth.php", "auth.triobit.net");
		...
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "Lightning");
		...
		{
			Auth_Error(va("Could not reach the triobit.net server. Error code from CURL: %x.", code));
		}
...
void Auth_OpenDLCStore()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://lightning.triobit.net/content/dlc.txt");
